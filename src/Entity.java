...
public class Entity extends JPanel{
	...
	public int w;
	public int h;
	protected boolean facingRight = false; //false = facing left, true = facing right
	...
	//Start the AI
	public void start(){
		new Thread(new Runnable(){ @Override public void run(){ animate(); } } ).start();
	}
	...
	protected void loadImage(String path, int w, int h){
		...
		try {
			...
			if(!facingRight) image = new JLabel(new ImageIcon(bi.getScaledInstance(w, h, Image.SCALE_SMOOTH)));
			else image = new JLabel(new ImageIcon(bi.getScaledInstance(-w, h, Image.SCALE_SMOOTH)));
			image.setSize(w, h);
			this.add(image);
			//this.revalidate();
			this.repaint();
	...
	protected void loadImage(String path, int w, int h, int x, int y){
		try {
			BufferedImage sprite = ImageIO.read(getClass().getResource(path)).getSubimage(x, y, w, h);
			if(!facingRight) image = new JLabel(new ImageIcon(sprite.getScaledInstance(w, h, Image.SCALE_SMOOTH)));
			else image = new JLabel(new ImageIcon(sprite.getScaledInstance(-w, h, Image.SCALE_SMOOTH)));
			image.setSize(w, h);
			this.removeAll();
			this.add(image);
		} catch (IOException e) { System.err.println("The specified image could not be loaded: " + path); }
	...
	protected JLabel loadTileImage(String path, int w, int h, int x, int y){
		try {
			BufferedImage sprite = ImageIO.read(getClass().getResource(path)).getSubimage(x*w, y*h, w, h);
